...
export function FeedStoryStrip({ items }: FeedStoryStripProps) {
  ...
  return (
    <div className="no-scrollbar -mx-1 flex gap-2.5 overflow-x-auto px-1 pb-0.5">
      {items.map((item) => (
        <Link
          key={item.userId}
        ...
        >
          <span className="inline-flex rounded-full bg-[linear-gradient(135deg,oklch(0.73_0.16_294),oklch(0.7_0.15_243))] p-[2px] shadow-[0_12px_28px_-16px_oklch(0.73_0.16_294_/_0.8)]">
            <Avatar className="size-12 border border-black/30 bg-background">
          ...
          <div className="flex flex-col">
            <span className="max-w-[4.2rem] truncate text-[11px] font-medium text-foreground">
              {item.displayName}
            </span>
            <span className="max-w-[4.2rem] truncate text-[10px] text-muted-foreground">
              @{item.username}
            </span>
          </div>
        </Link>
